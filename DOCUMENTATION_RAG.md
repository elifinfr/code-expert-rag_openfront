...
>
> Deux distributions existent :
> - **`rag_code_expert_ts_js`** — langages supportés : **TypeScript / JavaScript**.
> - **`rag_code_expert_last_build`** — ajoute le support **Python** (validé sur le
>   repo `click` de Pallets). Toute la section « multi-langage » s'applique aux deux ;
>   seule la liste des providers diffère.
